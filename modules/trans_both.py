...
#     d_k = query.size(-1)
#     scores = (torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)).float()
#     if mask is not None:
#         scores = scores.masked_fill(mask == 0, -1e9)
#         #scores = scores.masked_fill(mask == 0, 1e-3)
...
#         print(1111111111111, 'error in attention')
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        model = Transformer(
            ...
            Decoder(
                ...
                DecoderLayer(self.d_model, c(attn_decode), c(ff), self.dropout),
                self.num_layers_de),
            lambda x: x,
            fbl=self.fbl)
    ...
    def __init__(self, config, tokenizer):
        super(EncoderDecoder, self).__init__(config, tokenizer)
        self.config = config
        ...
        if self.fbl:
            self.num_patches += 1
        self.pe = config['pe']
        # self.cvt_attn = args.cvt_attn
        # self.stride_q = args.stride_q
        # self.stride_kv = args.stride_kv
        # self.padding_q = args.padding_q
        # self.padding_kv = args.padding_kv
        # self.kernel_size = args.kernel_size
        # self.bwinit = args.bwinit
    ...
    def _prepare_feature(self, att_feats, att_masks):
        ...
        return att_feats[..., :1], memory, att_masks
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        # print(torch.max(out[:,-1]),torch.min(out[:,-1]))
