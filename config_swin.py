...
_C.TRAIN.WEIGHT_DECAY = 5e-5 # origin 0.05
_C.TRAIN.VE_BASE_LR = 5e-5
_C.TRAIN.ED_BASE_LR = 1e-4
_C.TRAIN.WARMUP_LR = 5e-6
_C.TRAIN.MIN_LR = 5e-6
# Clip gradient norm
_C.TRAIN.CLIP_GRAD = 0.1 #5 origin
...
_C.LOCAL_RANK = os.environ['LOCAL_RANK']
...
def update_config(config, args):
    ...
    config.MODEL.NAME = args.exp_name
    if args.batch_size:
    ...
    if args.lr_ve:
        config.TRAIN.VE_BASE_LR = args.lr_ve
    if args.lr_ed:
        config.TRAIN.ED_BASE_LR = args.lr_ed
    if args.decay_rate:
        config.TRAIN.LR_SCHEDULER.DECAY_RATE = args.decay_rate
    if args.optim:
        config.TRAIN.OPTIMIZER.NAME = args.optim
    if args.amsgrad:
        config.TRAIN.OPTIMIZER.AMSGRAD = args.amsgrad
    if args.eps:
        config.TRAIN.OPTIMIZER.EPS = args.eps
    if args.cvt_attn:
        config.TRAIN.CVT_ATTN = args.cvt_attn
